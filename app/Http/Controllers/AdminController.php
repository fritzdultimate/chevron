...
class AdminController extends Controller {
    ...
    public function makeModerator(Request $request) {
        ...
        if($user->permission == '1') {
            ...
            if($make_user_moderator) {
                return back()->with('success', 'User has been upgraded to a Moderator');
            }
    
            return back()->with('error', 'Error making user a moderator');
        } 
    }
    public function removeModerator(Request $request) {
        ...
        if($remove_user_moderator) {
            return back()->with('success', 'User has been downgraded to a normal clients');
        }

        return back()->with('error', 'Error removing user as a moderator');
    }
