...
class HomeController extends Controller {
    ...
    public function InvestmentProcess(Request $request, $name) {
        ...
        // $wallets = collect($this->currencies())
    ...
    public function InvestmentDetails(Request $request, $id) {
        ...
        if(!$deposit) return to_route('user.select-plan');
    ...
    public function WithdrawalDetails(Request $request, $id) {
        ...
        if(!$withdrawal) return redirect('/user/all-transactions');
    ...
    public function allTransactions(Request $request){
        ...
        $transactions = Transactions::with(['deposit', 'withdrawal'])
            ->where('user_id', $user['id'])
            ->whereHas('deposit', fn($query) => $query->where('user_id', $user['id']))
            ->orWhereHas('withdrawal' , fn($query) => $query->where('user_id', $user['id']))
            ->latest()
            ->get();
        // dd($transactions);
    ...
    public function totalWithdrawn(Request $request){
        $page_title = env('SITE_NAME') . " Investment Website | Manage Total Withdrawn";
        $mode = 'dark';
        $user = Auth::user();
        $users = User::all();
        ...
        if($request->isMethod('post')){
            ...
            return view('user.total-withdrawn', compact('page_title', 'mode', 'user','users'));
        }
    }
    ...
    public function changePass(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if(!session('email')) return to_route('login');
            ...
            if($update_password) {
                return to_route('login')->with('success', "password has been reset successfully");
            }
        } else {
    ...
    public function verifyToken(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($request->token != $token) return back()->with('error', "invalid code or expired code");

            if($request->email != $email) return back()->with('error', "invalid email address");

            
            return to_route('guest.change-pass')->with('success', 'Token Verified');
