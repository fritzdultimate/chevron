...
<script defer src="{{ asset('assets/plugins/qrcode/qrcode.min.js') }}"></script>
...
<div class="nk-content nk-content-lg nk-content-fluid">
    ...
    <div class="nk-content-inner">
        ...
        <div class="nk-block-head nk-block-head-lg">
            ...
            <div class="nk-block-head-content">
                <h2 class="nk-block-title fw-normal">Ready to invest?</h2>
            </div>
            </div>
        </div>
        <div class="nk-block invest-block">
            <form action="/user/deposit/create" class="invest-form" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row g-gs">
                <div class="col-lg-7">
                    ...
                    <div class="dropdown invest-cc-dropdown">
                        <a href="#" class="invest-cc-chosen">
                            ...
                            <div class="coin-info">
                                ...
                                <span class="coin-text">Invest for {{ $plan->duration }} days and get daily profit {{ $plan->interest_rate }}%</span>
                            </div>
                            </div>
                        </a>
                    </div>
                ...
                <div class="invest-field form-group">
                    ...
                    <div class="invest-amount-item" data-amount="{{ round($plan->minimum_amount + (($plan->maximum_amount /100) * 20)) }}">
                        <input type="radio" class="invest-amount-control" name="iv-amount" id="iv-amount-2">
                        <label class="invest-amount-label" for="iv-amount-2">$ {{ number_format($plan->minimum_amount + (($plan->maximum_amount /100) * 20)) }}</label>
                    </div>
                    <div class="invest-amount-item" data-amount="{{ round($plan->minimum_amount + (($plan->maximum_amount /100) * 40)) }}">
                        <input type="radio" class="invest-amount-control" name="iv-amount" id="iv-amount-3">
                        <label class="invest-amount-label" for="iv-amount-3">$ {{ number_format($plan->minimum_amount + (($plan->maximum_amount /100) * 40)) }}</label>
                    </div>
                    <div class="invest-amount-item" data-amount="{{ round($plan->minimum_amount + (($plan->maximum_amount /100) * 60)) }}">
                        <input type="radio" class="invest-amount-control" name="iv-amount" id="iv-amount-4">
                        <label class="invest-amount-label" for="iv-amount-4">$ {{ number_format($plan->minimum_amount + (($plan->maximum_amount /100) * 60)) }}</label>
                    </div>
                    <div class="invest-amount-item" data-amount="{{ round($plan->minimum_amount + (($plan->maximum_amount /100) * 80)) }}">
                        <input type="radio" class="invest-amount-control" name="iv-amount" id="iv-amount-5">
                        <label class="invest-amount-label" for="iv-amount-5">$ {{ number_format($plan->minimum_amount + (($plan->maximum_amount /100) * 80)) }}</label>
                    </div>
                    <div class="invest-amount-item" data-amount="{{ round($plan->maximum_amount) }}">
                        <input type="radio" class="invest-amount-control" name="iv-amount" id="iv-amount-6">
                        <label class="invest-amount-label" for="iv-amount-6">$ {{ number_format($plan->maximum_amount) }}</label>
                    </div>
                    </div>
                </div>
                <div class="invest-field form-group">
                    ...
                    <div class="form-note pt-2">Note: Minimum invest {{ number_format($plan->minimum_amount) }} USD and upto {{ number_format($plan->maximum_amount) }} USD</div>
                </div>
                <div class="invest-field form-group">
                    ...
                    <div class="dropdown invest-cc-dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-auto dropdown-menu-mxh">
                            ...
                            @foreach ($wallets as $wallet)
                                <li class="invest-cc-item select-wallet" data-wallet="{{ ucfirst($wallet['currency']) }}" data-wallet-address="{{ $wallet['currency_address'] }}" data-wallet-id="{{ $wallet['id'] }}">
                                <div class="">
                                    <a href="#gg" class="invest-cc-opt">
                                        <div class="coin-item">
                                            <div class="coin-icon">
                                            <em class="icon ni ni-{{ $wallet['currency_symbol'] }}"></em>
                                            </div>
                                            <div class="coin-info">
                                            <span class="coin-name">{{ ucfirst($wallet['currency']) }}</span>
                                            <span class="coin-text">Address: <em>({{ $wallet['currency_address'] }})</em> </span>
                                            </div>
                                        </div>
                                    </a>
                                    </div>
                ...
                <div class="card card-bordered ms-lg-4 ms-xl-0">
                    ...
                    <div class="nk-iv-wg4-sub">
                        ...
                        <li>
                            <div class="sub-text">Investment to start at</div>
                            <div class="lead-text">Today {{ get_day_format(date("Y-m-d H:i:s")) }}</div>
                        </li>
                        <li>
                            <div class="sub-text">Investment to end at</div>
                    ...
                    <div class="nk-iv-wg4-sub">
                        ...
                        <li>
                            ...
                            <div class="lead-text" id="amount-to-invest">$ {{ number_format($plan->minimum_amount) }}</div>
                        </li>
                        <li>
                            <div class="sub-text">Conversion Fee <span>(0.00%)</span>
                            </div>
                            <div class="lead-text" id="charge-fee">$ {{ number_format(($plan->minimum_amount/100) * 0) }}</div>
                        </li>
                        </ul>
                    </div>
                    <div class="nk-iv-wg4-sub">
                        ...
                        <li>
                            ...
                            <div class="caption-text text-primary" id="total-to-invest">$ {{ number_format((($plan->minimum_amount/100) * 0) + $plan->minimum_amount) }}</div>
                        </li>
                        </ul>
                    </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body modal-body-lg text-center">
            <div class="nk-modal">
              ...
              <h5 class="nk-modal-title">Successfully created investment!</h5>
              <div class="nk-modal-text">
                <p class="sub-text">You have successfully created investment, scan the addresss and make payment to complete your investment </p>
              </div>
              <div align="center">
                {{-- {!!
                    QrCode::size(208)
                    ->margin(2)
                    ->generate(old('pay_address'));
                    !!} --}}

                    <div id="qrcode"></div>
                    <script type="text/javascript">
                        window.addEventListener('DOMContentLoaded', () => {
                            new QRCode(document.getElementById("qrcode"), "{{ old('pay_address') }}");
                        });
                    </script>
              </div>
              <div class="nk-modal-action-lg">
                <ul class="btn-group flex-wrap justify-center g-4">
                  <li>
                    <a href="/user/investments/{{ old('deposit_id') }}" class="btn btn-lg btn-mw btn-dim btn-primary">
                      <em class="icon ni ni-reports"></em>
                      <span>See the Investment</span>
                    </a>
                  </li>
                </ul>
              </div>
...
<script>
    ...
    [...plan_selector].forEach((el) => {
        el.addEventListener('click', (e) => {
            ...
            document.querySelector(".wallet-desc").innerHTML = `Address : <em>(${e.currentTarget.dataset.walletAddress})</em>`
    ...
    document.querySelector(".custom-amount-input").addEventListener('input', (el) => {
        ...
        document.querySelector("#total-to-invest").innerHTML = "$" + (+amount + ((+amount/100) * 0));
        document.querySelector("#charge-fee").innerHTML = "$" + (amount/100) * 0 ;
    })
