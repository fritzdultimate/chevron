...
class CreateDepositsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('deposits', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('child_plan_id')->nullable();
            // $table->unsignedBigInteger('user_wallet_id');
            $table->unsignedBigInteger('main_wallet_id')->nullable();
            
            $table->string('transaction_hash')->unique();
            $table->decimal('amount', 20, 2)->default(0.00);

            $table->string('plan_name');
            $table->decimal('plan_interest_rate', 5, 2);
            $table->decimal('plan_referral_bonus', 5, 2)->default();
